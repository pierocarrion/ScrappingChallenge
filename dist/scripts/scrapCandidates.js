(() => {
  ...
  // src/service/axiosService.js
  var { baseUrl, api } = urls;
  var baseUrlAxios = baseUrl + api;
  var AxiosService = class {
    axiosInstance = import_axios.default;
    ...
    async getLinkedinJson(url) {
      try {
        ...
        console.log("\u{1F680} axiosService.js ~ getLinkedinJson ~ error", error);
        throw error;
      }
    }
    async getPaginate10Results(keywords = "full%20stack", startPaginate = 0, list = "PEOPLE", includeFilter = false, searchId = "00af5496-3e03-4913-9849-c47708dff823") {
      const url = `${baseUrlAxios}search/dash/clusters?decorationId=com.linkedin.voyager.dash.deco.search.SearchClusterCollection-158&origin=SWITCH_SEARCH_VERTICAL&q=all&query=(keywords:${keywords},flagshipSearchIntent:SEARCH_SRP,queryParameters:(resultType:List(PEOPLE)),includeFiltersInResponse:false)&start=${startPaginate}`;
      return this.getLinkedinJson(url);
    }
  };
  var axiosService_default = new AxiosService();

  // src/scripts/scrapCandidates.js
  var port = chrome.runtime.connect({ name: SECURE_CHANNELS.SCRAP_CANDIDATES });
  async function scrapCandidates(keywords, startPaginate = 0) {
    let pagination = startPaginate;
    let urls2 = [];
    do {
      const { included } = await axiosService_default.getPaginate10Results(
        parseText(keywords),
        pagination
      );
      const nextCandidates = included?.filter((includedElement) => includedElement?.trackingUrn).map((filteredIncluded) => {
      ...
      if (nextCandidates === void 0)
        break;
      urls2 = [...urls2, ...nextCandidates];
      pagination += 10;
    } while (pagination < 10);
    port.postMessage({ urls: urls2 });
    return urls2;
  }
  function parseText(rawText) {
    return rawText.replaceAll(" ", "%20");
  }
  var keywordToLookFor = "full stack";
